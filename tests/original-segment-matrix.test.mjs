...
test("one uses separate G and H phosphor electrodes with a narrow gap",()=>{const level=2;const gap=minimumMaskGap(ORIGINAL_SEGMENT_MASKS.G,ORIGINAL_SEGMENT_MASKS.H,level);assert.ok(gap>=0);assert.ok(gap<=3,`G/H gap too large: ${gap}`);const g=activeCellsForSegment("G",ORIGINAL_SEGMENT_MASKS,level),h=activeCellsForSegment("H",ORIGINAL_SEGMENT_MASKS,level);assert.ok(Math.max(...g.map(c=>c.row))<Math.min(...h.map(c=>c.row)))});
